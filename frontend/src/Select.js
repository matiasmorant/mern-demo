...
const Searchbox = styled.div`
  ...
  align-items: center;
	...
	padding-left: 5px;
  button, input, img, .top-icon-container{
...
class Select extends Component {
  ...
  changeSelectedOption=(index, text)=>{
    this.setState({selectedOption: index, text: text||this.props.options[index]||""})
    this.props.onOptionChange(this.props.options[index])
  };
  handleTextChange=(text)=>{
    let trimmed = text.trim()
    let index = trimmed ? this.props.options.findIndex((option)=>option.toUpperCase().startsWith(trimmed.toUpperCase())) : -1;
  ...
  handleKeyDown=(event)=>{
  	...
  	if(key === "Enter"||key === "Escape"){
  		...
  		this.input.blur()
  	} else if(key === "ArrowDown" && selectedOption+1<this.optionComponents.length ){
  ...
  componentDidUpdate() {
    let {selectedOption} = this.state;
    if(selectedOption >= 0){
    	let {offsetTop, offsetHeight} = this.optionComponents[selectedOption]
    	let {scrollTop, offsetHeight: scrollHeight} = this.dropdown
  ...
  render() {
    return (
      <Selector {...this.props}>
        <Searchbox>
          <button onFocus={()=>this.input.focus()}><i className="fa fa-caret-down"></i></button>
          <div className="top-icon-container">
          	<img alt="" src={this.props.icons[this.state.selectedOption] || this.props.defaultIcon}/>
          </div>
          <input
           ...
           value={
           	this.state.showDropdown ? this.state.text :
        ...
        <Dropdown
         ...
         {this.props.options.map((c, i)=>
          <DropdownItem 
           ...
           onMouseDown={(e)=>{this.handleOptionSelection(i)}}>
